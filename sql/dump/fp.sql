...
--
-- Host: localhost    Database: farm_parallel
-- ------------------------------------------------------
...
CREATE TABLE `dv_sensor_data` (
  `sensor_data_seq` int(11) NOT NULL AUTO_INCREMENT COMMENT '센서 데이터 시퀀스',
  `node_seq` int(11) NOT NULL COMMENT '노드 정보 시퀀스',
...
/*!50001 CREATE TABLE `v_pw_inverter_profile` (
  ...
  `address` tinyint NOT NULL,
  `ip` tinyint NOT NULL,
  `push_port` tinyint NOT NULL,
  `cmd_port` tinyint NOT NULL,
...
/*!50013 DEFINER=`fp`@`%` SQL SECURITY DEFINER */
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`fp`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `v_dv_place` AS select `p`.`place_seq` AS `place_seq`,`p`.`place_def_seq` AS `place_def_seq`,`pc`.`place_class_seq` AS `place_class_seq`,case when octet_length(`p`.`target_code`) > 0 then concat(`pd`.`target_prefix`,'_',`p`.`target_code`) else concat(`pd`.`target_prefix`) end AS `place_id`,case when octet_length(`p`.`target_code`) > 0 then concat(`pd`.`target_name`,' ',`p`.`target_code`) else concat(`pd`.`target_name`) end AS `place_name`,`p`.`depth` AS `depth`,`p`.`place_info` AS `place_info`,`p`.`target_code` AS `p_target_code`,`pd`.`target_prefix` AS `pd_target_prefix`,`pd`.`target_id` AS `pd_target_id`,`pd`.`target_name` AS `pd_target_name`,`pc`.`target_id` AS `pc_target_id`,`pc`.`target_name` AS `pc_target_name`,`pc`.`description` AS `pc_description` from ((`dv_place` `p` join `dv_place_def` `pd` on(`pd`.`place_def_seq` = `p`.`place_def_seq`)) join `dv_place_class` `pc` on(`pc`.`place_class_seq` = `pd`.`place_class_seq`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
